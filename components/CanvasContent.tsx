        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.projects?.map((project: any) => (
                <div key={project.id} className="bg-brand-night rounded-xl overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:shadow-brand-accent/20">
                    <img src={project.imageUrl} alt={project.title?.[locale]} className="w-full h-56 object-cover group-hover:scale-105 transition-transform duration-300" />
                    <div className="p-6">
                        ...
                        <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-brand-accent font-semibold">View Project &rarr;</a>
                    </div>
                </div>
            ))}
        </div>
...
const ContactSection: React.FC<{ data: any; locale: Locale, effects: any }> = ({ data, locale, effects }) => (
    <div className={`py-24 px-6 text-center relative ${effects.blur ? 'bg-brand-night/50 backdrop-blur-md' : 'bg-brand-night'}`}>
        ...
        <div className="flex justify-center gap-6 mt-8">
            <a href={data.socialLinks?.github} className="text-brand-mist hover:text-white"><Github size={28}/></a>
            <a href={data.socialLinks?.linkedin} className="text-brand-mist hover:text-white"><Linkedin size={28}/></a>
            <a href={data.socialLinks?.twitter} className="text-brand-mist hover:text-white"><Twitter size={28}/></a>
        </div>
    </div>
);
